from django.db import models
from django.contrib.auth.models import User, UserManager
from django.utils import timezone
from django.db.models import Sum


# Create your models here.

class QuestionManager(models.Manager):
    def hot(self):
        return self.order_by('-rating')

    def newest(self):
        return self.order_by('-date')

    def byTag(self, input_tag):
        return self.filter(tags__text=input_tag)

    def byUser(self, user_name):
        return self.filter(author__username=user_name)


class ProfileManager(models.Manager):
    def getName(self, user_name):
        return self.filter(user__username=user_name)

    def getUsername(self, id):
        return self.filter(user__id=id)


class QuestionLikeManager(models.Manager):
    def filter_by_question(self, question):
        return self.filter(question=question)

    def sum_likes(self, question):
        return self.filter_by_question(question).aggregate(sum=Sum('rating'))['sum']

    def add_or_update(self, author, question, value):
        new = self.update_or_create(author=author, question=question, defaults={'rating': value})

        question.rating = self.sum_likes(question)
        question.save()
        return new


class AnswerLikeManager(models.Manager):
    def filter_by_answer(self, answer):
        return self.filter(answer=answer)

    def sum_likes(self, answer):
        return self.filter_by_answer(answer).aggregate(sum=Sum('rating'))['sum']

    def add_or_update(self, author, answer, value):
        new = self.update_or_create(author=author, answer=answer, defaults={'rating': value})

        answer.rating = self.sum_likes(answer)
        answer.save()
        return new


class Tag(models.Model):
    class Meta:
        verbose_name = u'Tag'
        verbose_name_plural = u'Tags'

    text = models.CharField(max_length=15, verbose_name='Tag')

    def __str__(self):
        return self.text


class Question(models.Model):
    class Meta:
        verbose_name = u'Question'
        verbose_name_plural = u'Questions'

    id = models.IntegerField(unique=True, primary_key=True)
    author = models.ForeignKey(User, verbose_name=u'User')
    title = models.CharField(max_length=100, verbose_name=u'title')
    text = models.TextField(verbose_name=u'Text')
    rating = models.IntegerField(default=0, verbose_name=u'Rating')
    count_answers = models.IntegerField(default=0, verbose_name=u'Count of answers')
    tags = models.ManyToManyField(Tag, verbose_name=u'tag')
    isVote = models.BooleanField(default=True, verbose_name=u'Vote')
    date = models.DateTimeField(default=timezone.now, verbose_name=u'Date')

    objects = QuestionManager()

    def __str__(self):
        return self.text


class Answer(models.Model):
    class Meta:
        verbose_name = u'Answer'
        verbose_name_plural = u'Answers'

    author = models.ForeignKey(User, verbose_name=u'User')
    question = models.ForeignKey(Question, verbose_name=u'Question')
    text = models.TextField(verbose_name=u'Text')
    rating = models.IntegerField(default=0, verbose_name=u'rating')
    is_correct = models.BooleanField(default=False, verbose_name=u'Is correct')
    date = models.DateTimeField(default=timezone.now, verbose_name=u'Date')


class Profile(models.Model):
    class Meta:
        verbose_name = u'Profile'
        verbose_name_plural = u'Profiles'

    user = models.OneToOneField(User, verbose_name=u'User')
    avatar = models.ImageField(upload_to='avatars')

    objects = ProfileManager()

    def __str__(self):
        return self.user


class QuestionLike(models.Model):
    class Meta:
        verbose_name = u'Rating of question'
        verbose_name_plural = u'Rating of questions'

    question = models.ForeignKey(Question, default=False, verbose_name=u'Question')
    author = models.ForeignKey(User, verbose_name=u'User')
    rating = models.IntegerField(default=0, db_index=True, verbose_name=u'Rating')

    objects = QuestionLikeManager()

    def __str__(self):
        return self.rating


class AnswerLike(models.Model):
    class Meta:
        verbose_name = u'Answer rating'
        verbose_name_plural = u'Answers rating'

    answer = models.ForeignKey(Answer, default=False, verbose_name=u'Answer')
    author = models.ForeignKey(User, verbose_name=u'User')
    rating = models.IntegerField(default=0, db_index=True, verbose_name=u'Rating')

    objects = AnswerLikeManager()

    def __str__(self):
        return self.rating